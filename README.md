...
Dataset repository for the paper: **AQUAPOSE: A Large Scale Annotated Dataset and Benchmark for 2D Human Pose Estimation in Underwater Scenes** **(insert author list)**
...
## Structures
The main folder `dataset` contains 94 subfolders named from `1` to `94`. Each subfolder contains two secondary subfolders 'images'(video frames of swimming, each of them in `png` format) and 'annots'(information about `18 keypoints` and `bounding box` of swimmers' body, each of them in `json` file format) 
